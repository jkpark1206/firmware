...
class TPLinkZHSpider(Spider):
    ...
    def parse(self, response):
        ...
        for page in range(1, maxPage+1):
            #url = urljoin(response.url, '/download1/readmore')
    ...
    def parse_json(self, response):
        ...
        for product in resp:
            ...
            url = "https://service.tp-link.com.cn/download/" + product['fileName']
            #self.logger.debug(url)
            #self.logger.debug(product['fileName'])
            item.add_value("url", url)
            item.add_value("description", name)
            item.add_value("vendor", self.vendor)
            #if "升级软件" not in product['fileName']:
            #    self.logger.debug("Mark")
            #    self.logger.debug(product['fileName'])
